    ...
    if contracts:
        for contract in contracts:
            try:
                ...
                logger.info("There is a problem with the contracts tasks")
    ...
    if bids:
        for bid in bids:
            if bid.endDate-7 == datetime.now().date():
                offers = bid.offers.all()
                postulations = bid.postulations.all()
                if offers :
                ...
                if postulations:
                    for postulation in postulations:
                        carrier = Carrier.objects.get(id = postulation.carrier)
